...
class ProposalsTargetLayer(object):
    ...
    def sample_rois_img(self, elems=None):
        ...
        fgd_prds_pre = bbox_transform(fgd_rois, fgd_prds_tmp)         #和fgd_idxs对应 #(M0, 4)
        ###Get MASK###
        #ROIS相对于gt_boxes的位置
        ...
        ####################Get Losses###################
        fbg_prbs_pst, fbg_prds_pst, fbg_msks_pst = self.RP.rois_pooling_img_vld(roi_scrs_pst, roi_prds_pst, fbg_rois) #(M, C)
        
        ###Get Class Loss###
        roi_prbs_los = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=fbg_prbs_pre, logits=fbg_prbs_pst) #(M) #和fbg_idxs对应
        ###Get Mask Loss###
        ...
        roi_msks_los = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=fgd_msks_pre, logits=fgd_msks_pst) #(M0, 21, 21)
        roi_msks_los = tf.reduce_mean(roi_msks_los, axis=[1, 2])       #reduce_mean还是reduce_sum
        #roi_msks_los = tf.reduce_sum(roi_msks_los, axis=[1, 2])       #(M0)
        roi_loss_tmp = tf.zeros(shape=[bgd_num], dtype=tf.float32)
        roi_msks_los = tf.concat([roi_msks_los, roi_loss_tmp], axis=0) #(M)
        ###Get Bbox loss###
        ...
        roi_prds_los = tf.reduce_sum(roi_prds_los, axis=1)             #(M0)
        roi_prds_los = tf.concat([roi_prds_los, roi_loss_tmp], axis=0) #(M)
        ###Get Total loss###
        roi_loss     = roi_prbs_los + roi_prds_los + roi_msks_los*10.0 #(M) #和fbg_idxs对应
        roi_loss, fbg_ixxs = tf.nn.top_k(roi_loss, k=self.roi_fbg_num, sorted=False) #fbg_idxs中的前背景顺序没有乱
        ###重新定位前背景ROI###
        fbg_idxs     = tf.gather(fbg_idxs, fbg_ixxs)
        fbg_num      = tf.shape(fbg_idxs)[0]
        fbg_rois     = tf.gather(fbg_rois, fbg_ixxs)
        ...
        fgd_num      = tf.shape(fgd_idxs)[0]
        fgd_msks_pre = tf.gather(fgd_msks_pre, fgd_ixxs)
        fgd_prds_pre = tf.gather(fgd_prds_pre, fgd_ixxs)
        
        paddings     = [[0, self.roi_fbg_num-fbg_num], [0, 0]]
    ...
    def sample_rois(self, rois, roi_nums, gbxs, gmks, gbx_nums, roi_scrs_pst, roi_prds_pst):
        
        elems = [rois, roi_nums, gbxs, gmks, gbx_nums, roi_scrs_pst, roi_prds_pst]
    ...
    def generate_roi_loss(self, roi_prbs_pst=None, roi_prds_pst=None, roi_msks_pst=None):
        ...
        roi_prbs_los = tf.cond(fgd_num>0, lambda: roi_prbs_los/fgd_num, lambda: tf.constant(0.0))
        roi_prds_los = tf.cond(fgd_num>0, lambda: roi_prds_los/fgd_num, lambda: tf.constant(0.0))
        roi_msks_los = tf.cond(fgd_num>0, lambda: roi_msks_los/fgd_num, lambda: tf.constant(0.0)) * 10.0
