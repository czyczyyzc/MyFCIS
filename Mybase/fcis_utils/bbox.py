...
def bbox_intersects(boxs, gbxs):
    ...
    int_ymn = tf.maximum(box_ymn, gbx_ymn)
    int_xmn = tf.maximum(box_xmn, gbx_xmn)
    int_ymx = tf.minimum(box_ymx, gbx_ymx)
    int_xmx = tf.minimum(box_xmx, gbx_xmx)
    
    int_hgt = tf.maximum(int_ymx-int_ymn, 0.)
    int_wdh = tf.maximum(int_xmx-int_xmn, 0.)
    int_ara = int_hgt * int_wdh
    
    box_ara = (box_ymx - box_ymn) * (box_xmx - box_xmn)
    iscs = int_ara / box_ara
    iscs = tf.reshape(iscs, [box_num, gbx_num])
    return iscs


def bbox_intersects_1(boxs, gbxs):
...
def bbox_intersects_py(boxs, gbxs):
    ...
    int_ymn = np.maximum(box_ymn, gbx_ymn)
    int_xmn = np.maximum(box_xmn, gbx_xmn)
    int_ymx = np.minimum(box_ymx, gbx_ymx)
    int_xmx = np.minimum(box_xmx, gbx_xmx)
    
    int_hgt = np.maximum(int_ymx-int_ymn, 0.)
    int_wdh = np.maximum(int_xmx-int_xmn, 0.)
    int_ara = int_hgt * int_wdh
...
def bbox_overlaps(boxs, gbxs):
    ...
    box_ara = (box_ymx - box_ymn) * (box_xmx - box_xmn)
    gbx_ara = (gbx_ymx - gbx_ymn) * (gbx_xmx - gbx_xmn)
    uin_ara = box_ara + gbx_ara - int_ara #union_area
    
    ovps = int_ara / uin_ara
